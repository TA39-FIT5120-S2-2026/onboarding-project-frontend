...
## Implementation status (first build pass)

What the first build pass actually verified, and what still needs a human with a browser and a screen reader before a card moves to Done. Automated checks below ran as part of `npm test` / `npm run lint`; nothing here substitutes for the manual pass on the checklist above.
...
| Warnings use `role="alert"` | `CrowdWarning`, `ToleranceWarning`, `NoQualifyingRouteMessage`, `PredictiveAlert`, inline form errors all render `role="alert"`; asserted in their tests | Automated |
...
| `prefers-reduced-motion` respected | Global CSS rule in `index.css` collapses all transition/animation durations; Leaflet's `fitBounds` calls always pass `animate: false` | Code-reviewed |
