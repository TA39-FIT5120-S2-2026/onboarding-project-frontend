...
| Map markers have accessible names | `RefugeMarker`, and `RouteMap`'s start/destination markers, set a real `aria-label` on the Leaflet marker element via `getElement()`, not just the divIcon's `alt` string. Start/destination are also distinct by shape (filled circle vs pin), not colour alone. jsdom cannot run Leaflet's renderer, so this is code-reviewed, not test-covered; the redundant text list next to every map (and the "Start"/"Destination" legend entries) is the accessibility-guaranteed path | Code-reviewed + partially automated |
